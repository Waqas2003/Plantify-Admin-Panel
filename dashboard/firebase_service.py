...
class FirebaseRepository:
    ...
    def add_community(self, data):
        """Add a new community document and return the document reference"""
        doc_ref = self.db.collection('Communities').document()
        doc_ref.set(data)
        return doc_ref
    ...
    def get_community_members(self, community_id):
        members = []
        members_ref = self.db.collection('Communities').document(community_id.strip()).collection('Members').stream()
    ...
    def get_banned_users(self, community_id):
        banned_users = []
        banned_ref = self.db.collection('Communities').document(community_id.strip()).collection('BannedUsers').stream()
    ...
    def get_user(self, user_email):
        return self.db.collection('Users').document(user_email.strip()).get()

    def get_community_members_count(self, community_id):
        members_ref = self.db.collection('Communities').document(community_id.strip()).collection('Members').get()
        return len([doc for doc in members_ref if doc.id != 'placeholder'])
    ...
    # def get_community_members_count(self, community_id):
    #     members_ref = self.db.collection('Communities').document(community_id).collection('Members').get()
    #     return len([doc for doc in members_ref if doc.id != 'placeholder'])    
